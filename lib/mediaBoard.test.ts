...
describe('fetchMediaBoard', () => {
  ...
  it('нормализует items из ответа функции', async () => {
    invoke.mockResolvedValue({
      data: { items: [
        { name: 'Кафе Onion', coords: [37.5, 127.0], segment: 'food', rubric: 'new', blurb: 'модное', source: 'Time Out Seoul', sourceDate: '2025' },
        { name: 'Смотровая', coords: [37.55, 127.02], segment: 'sight', rubric: 'best' },
      ] },
      error: null,
    });
    ...
    expect(invoke).toHaveBeenCalledWith('trends', { body: { city: 'Токио', country: 'Япония' } });
  });

  it('место без координат сохраняется (coords=null), кривая рубрика → trending', async () => {
    invoke.mockResolvedValue({ data: { items: [{ name: 'Без точки', coords: null, rubric: 'xxx' }] }, error: null });
  ...
  it('ошибка функции / тело error / не массив → []', async () => {
  ...
  it('элемент без имени отбрасывается', async () => {
    invoke.mockResolvedValue({ data: { items: [{ name: '', coords: [1, 2] }, { name: 'Ok', coords: [1, 2] }] }, error: null });
    const r = await fetchMediaBoard('Токио');
