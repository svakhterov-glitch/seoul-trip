...
- **Edge Functions** (`supabase/functions/`, Deno/TypeScript). `npm test` их НЕ
  ...
  - **`telegram-webhook` — ЗАДЕПЛОЕН (Telegram-предложка).** Вебхук бота
    **@tripsplan_bot** в группе. Проверяет секрет в заголовке
    ...
    (`tg_links`). Режимы сообщения: ГЕОТЕГ Telegram (`location`/`venue` → ТОЧНЫЕ
    координаты от Telegram, без геокодинга; у venue ещё название/адрес), СПИСОК
    (нумерованные/маркированные строки или строки со ссылкой → каждый пункт, без
    лимита), одиночная ССЫЛКА (resolve-link: фото/координаты/описание),
    ФОТО/ПЕРЕСЛАННОЕ без ссылки (Haiku-разбор + фото в бакет `tg-photos`). Место без
    координат после клиентского разбора помечается «?» в предложке. Классификация
    место|покупка по домену (oliveyoung/coupang/…).
