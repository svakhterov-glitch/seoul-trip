...
## 4. Авторизация (для синхронизации БД)

Таблица `trips` защищена RLS — каждый видит только свои поездки, для записи
нужен вход. Экран входа (Supabase Auth) — **следующий шаг разработки**.
Пока для быстрой проверки синхронизации можно временно войти анонимно или
через свой токен; для продакшена добавим вход по email/OAuth.

> Только разбор ссылок и ИИ-генерацию можно включить **без БД и входа** —
> достаточно задеплоить функции и указать `functionsUrl` (блок `supabase`
> в конфиге можно не заполнять — останется localStorage).
