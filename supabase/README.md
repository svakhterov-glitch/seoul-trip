...
Workflow лежит готовым в `supabase/ci/deploy-supabase.yml`. Чтобы активировать,
скопируй его в `.github/workflows/` и запушь (токену нужен `workflow`-scope:
`gh auth refresh -h github.com -s workflow`):

```bash
mkdir -p .github/workflows
cp supabase/ci/deploy-supabase.yml .github/workflows/
git add .github && git commit -m "Активировать деплой через Actions" && git push
```

Добавь секреты репозитория
